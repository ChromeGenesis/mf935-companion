...
/// Outside-tap dismissal is handled explicitly here: the outer detector
/// covers the whole surface and pops, the inner one absorbs taps on the
/// card itself. Barrier-only dismissal is unreliable across platforms.
    ...
    return GestureDetector(
      ...
      child: Center(
        child: GestureDetector(
          // Absorb taps inside the card so they don't reach the dismiss
          // detector wrapping the whole surface.
          onTap: () {},
          child: Material(
...
Future<T?> showGlassModal<T>(
  ...
  return showDialog<T>(
    context: context,
    // Outside tap always dismisses — no trapped modals. The modal itself
    // also handles outside-taps via its own detector (see [GlassModal]).
